...
//TODO Dave, Throughout, please number the section headings so that we can clearly see the progression of tasks. I've started with #1 for the following heading.

=== 1. Review and verify the AWS infrastructure

//TODO Dave, Please add a, b, etc., in the headings for the substeps. I've started with a & b in the following headings.

==== a. Confirm that the cluster instances are running

//TODO Dave, Within each subsection, number the steps so that we can easily follow. I've started with periods for the following steps:

. Open the AWS EC2 console.
. Filter on the stack name.
. Clear the running instance filter, and confirm that
the number of instances for the cluster is as expected (four in this example).

//TODO Dave, In that preceding sentence, what example are we referring to?

==== b. Verify that the provisioning instance has stopped

//TODO Dave, Please use periods for steps here as I've done above (and throughout the rest of the doc).
...
==== (Optional) Verify EBS encryption with a CMK
...
==== (Optional) Verify the KMS CMK policy
