...
* (Optional) A Network Load Balancer to manage connections to the Qumulo cluster.
...
//TODO Dack, I moved the NLB icon from where it was (half in and half out of the public subnet) to where it normally goes, and I've used our standard label. Anything incorrect here?
//Marcia, that is incorrect.  The NLB we deploy for public management connectivity is not being used to load balance between AZs.  It is simply connected to all the nodes in the cluster so regardless of any node state a customer can get to the Qumulo UI.  My drawing was correct as was my text.  I've put them back in place.
